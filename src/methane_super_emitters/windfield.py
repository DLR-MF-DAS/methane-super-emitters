...
def download_windfield(input_file, output_file, tmp_dir):
    ...
    latitude = data['lat']
    longitude = data['lon']
    area = [float(latitude.min()), float(longitude.min()),
    ...
    with netCDF4.Dataset(tmp_file, 'r') as fd:
        ...
        np.savez(output_file, methane=data['methane'], lat=data['lat'], lon=data['lon'],
                 qa=data['qa'], time=data['time'], mask=data['mask'], non_destriped=data['non_destriped'],
                 u=u_ups, v=v_ups)
