...
def s2_download(npz_file, output_dir):
    ...
    try:
        ...
        logging.warning(
            f"No S2 data find matching the query constructed from {npz_file}"
        )
        ...
        for row in range(0, rows, 16):
            for col in range(0, cols, 16):
                if row + 32 < rows and col + 32 < cols:
                    ...
                    original = fd["PRODUCT/methane_mixing_ratio_bias_corrected"][:][0][
                        row : row + 32
                    ][:, col : col + 32]
                    lat_window = fd["PRODUCT/latitude"][:][0][row : row + 32][
                        :, col : col + 32
                    ]
                    lon_window = fd["PRODUCT/longitude"][:][0][row : row + 32][
                        :, col : col + 32
                    ]
                    lat_bounds_window = fd[
                        "PRODUCT/SUPPORT_DATA/GEOLOCATIONS/latitude_bounds"
                    ][:][0][row : row + 32][:, col : col + 32]
                    lon_bounds_window = fd[
                        "PRODUCT/SUPPORT_DATA/GEOLOCATIONS/longitude_bounds"
                    ][:][0][row : row + 32][:, col : col + 32]
                    qa_window = fd["PRODUCT/qa_value"][:][0][row : row + 32][
                        :, col : col + 32
                    ]
                    u10_window = fd["PRODUCT/SUPPORT_DATA/INPUT_DATA/eastward_wind"][:][
                        0
                    ][row : row + 32][:, col : col + 32]
                    v10_window = fd["PRODUCT/SUPPORT_DATA/INPUT_DATA/northward_wind"][
                        :
                    ][0][row : row + 32][:, col : col + 32]
                    sza_window = fd[
                    ...
                    raa_window = np.where(
                        f_delta < 0, np.abs(f_delta + 180.0), np.abs(f_delta - 180.0)
                    )
                    raa_window = abs(180.0 - raa_window)
                    scattering_angle = -np.cos(np.radians(sza_window)) * np.cos(
                        np.radians(vza_window)
                    ) + np.sin(np.radians(sza_window)) * np.sin(
                        np.radians(vza_window)
                    ) * np.cos(
                        np.radians(raa_window)
                    )
                    sa_std_window = fd[
                        "PRODUCT/SUPPORT_DATA/INPUT_DATA/surface_altitude_precision"
                    ][:][0][row : row + 32][:, col : col + 32]
                    cloud_fraction_window = fd[
                        "PRODUCT/SUPPORT_DATA/INPUT_DATA/cloud_fraction_VIIRS_SWIR_IFOV"
                    ][:][0][row : row + 32][:, col : col + 32]
                    cirrus_reflectance_window = fd[
                        "PRODUCT/SUPPORT_DATA/INPUT_DATA/reflectance_cirrus_VIIRS_SWIR"
                    ][:][0][row : row + 32][:, col : col + 32]
                    methane_ratio_std_window = fd[
                        "PRODUCT/SUPPORT_DATA/INPUT_DATA/methane_ratio_weak_strong_standard_deviation"
                    ][:][0][row : row + 32][:, col : col + 32]
                    methane_precision_window = fd[
                        "PRODUCT/methane_mixing_ratio_precision"
                    ][:][0][row : row + 32][:, col : col + 32]
                    surface_albedo_window = fd[
                        "PRODUCT/SUPPORT_DATA/DETAILED_RESULTS/surface_albedo_SWIR"
                    ][:][0][row : row + 32][:, col : col + 32]
                    surface_albedo_precision_window = fd[
                        "PRODUCT/SUPPORT_DATA/DETAILED_RESULTS/surface_albedo_SWIR_precision"
                    ][:][0][row : row + 32][:, col : col + 32]
                    aerosol_optical_thickness_window = fd[
                        "PRODUCT/SUPPORT_DATA/DETAILED_RESULTS/aerosol_optical_thickness_SWIR"
                    ][:][0][row : row + 32][:, col : col + 32]
                    times = fd["PRODUCT/time_utc"][:][0][row : row + 32]
                    try:
                        parsed_time = [
                            datetime.datetime.fromisoformat(time_str[:19])
